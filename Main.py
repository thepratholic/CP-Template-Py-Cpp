...
# Corrected DSU functions that accept DSU arrays as parameters.
def dsu_find(a, parent):
    if parent[a] == a:
        return a
    parent[a] = dsu_find(parent[a], parent)
    return parent[a]

def dsu_union(a, b, parent, size):
    leader_a = dsu_find(a, parent)
    leader_b = dsu_find(b, parent)
